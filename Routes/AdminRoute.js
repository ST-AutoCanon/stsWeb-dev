...
router.get('/trackattendance', (req, res) => {
    const sql="SELECT emp_name,logged_in_location,logged_in_time,TIME_FORMAT(logged_in_time, '%H:%i:%s') AS login_time,logged_out_time,TIME_FORMAT(logged_out_time, '%H:%i:%s') AS logout_time,FLOOR(TIMESTAMPDIFF(SECOND, logged_in_time, logged_out_time) / 3600) AS hours,MOD(TIMESTAMPDIFF(SECOND, logged_in_time, logged_out_time) / 60, 60) AS minutes FROM attendance";
